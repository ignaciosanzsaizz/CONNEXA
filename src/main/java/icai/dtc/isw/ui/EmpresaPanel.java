...
public class EmpresaPanel extends JPanel {
    ...
    private JPanel contentCard; // contenedor dinámico

    public EmpresaPanel(User user, String[] sectores) {
    ...
    private void cargarEstado() {
        ...
        gbc.gridy = 0; gbc.insets = new Insets(8, 12, 8, 12);
        contentCard.add(title, gbc);

        if (emp == null) {
            // Estado inicial: aún no hay empresa → CTA para completar
            ...
            // Ya existe la empresa: SOLO acciones (sin ficha y SIN editar)
            JButton btnAnuncio = UIUtils.primaryButton("Poner anuncio"); // placeholder
    ...
    private void mostrarFormulario() {
        ...
        if (r == JOptionPane.OK_OPTION) {
            ...
            if (ok) {
                ...
                cargarEstado(); // refresca a estado con "Poner anuncio" únicamente
            } else {
